...
const TreatmentCategories = () => {
  ...
  return (
    <section className="bg-gradient-to-b from-white via-gray-500/60 to-white   py-14 sm:py-16 lg:py-24 px-2 sm:px-4 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-base sm:text-lg text-gray-600 max-w-2xl mx-auto px-4 font-normal">
          ...
          {categoryPair.map((category) => {
            ...
            return (
              <section
                key={category.id}
                id={category.id}
                className="flex-1 bg-gray-50/80 border border-gray-300/60 shadow-lg rounded-2xl sm:rounded-3xl px-4 py-8 sm:px-6 sm:py-10 lg:py-12 flex flex-col items-center hover:shadow-xl hover:bg-gray-50 transition-all duration-300"
                style={{ minWidth: 0 }}
              >
                <h3 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-7 text-center tracking-tight">
                  {category.title}
                </h3>
                <div className="flex flex-col items-center w-full">
                  <div
                    className="relative w-full max-w-2xl h-60 sm:h-72 lg:h-80 mb-5 group rounded-2xl sm:rounded-3xl shadow-lg border border-gray-300/50 bg-white overflow-hidden transition-all duration-300 hover:shadow-xl"
                    onMouseEnter={() => setPaused((prev) => ({ ...prev, [category.id]: true }))}
                    onMouseLeave={() => setPaused((prev) => ({ ...prev, [category.id]: false }))}
                  >
                    {/* Left arrow */}
                    <button
                      className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 z-10 bg-white/95 hover:bg-gray-700 hover:text-white text-gray-700 rounded-full p-2 border border-gray-300/60 shadow-lg transition-all duration-200 backdrop-blur-sm"
                    ...
                    {/* Right arrow */}
                    <button
                      className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 z-10 bg-white/95 hover:bg-gray-700 hover:text-white text-gray-700 rounded-full p-2 border border-gray-300/60 shadow-lg transition-all duration-200 backdrop-blur-sm"
                    ...
                    >
                      <div className="relative w-full h-full">
                        <Image
                          ...
                          className="object-cover rounded-2xl sm:rounded-3xl group-hover:scale-105 transition-transform duration-500"
                          loading={activeIndex === 0 ? "eager" : "lazy"}
                          placeholder="blur"
                          blurDataURL="data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD/4gHYSUND..."
                        />
                      </div>
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-gray-800/85 via-gray-700/60 to-transparent p-4 sm:p-6 rounded-b-2xl sm:rounded-b-3xl">
                  ...
                  <div className="flex gap-2 sm:gap-3 justify-center mb-3 sm:mb-4">
                    {category.conditions.map((_, idx) => (
                      <button
                        key={idx}
                        className={`w-3 h-3 sm:w-4 sm:h-4 rounded-full border shadow-md focus:outline-none transition-all duration-200 ${idx === activeIndex
                            ? "bg-gray-700 border-gray-700 scale-110 shadow-lg"
                            : "bg-gray-300/60 border-gray-400/60 hover:bg-gray-400/80"
                          }`}
                  ...
                  <p className="text-gray-600 text-sm sm:text-base lg:text-lg text-center max-w-xl mt-2 px-4 font-normal leading-relaxed">
